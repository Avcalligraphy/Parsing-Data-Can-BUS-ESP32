...
void manageWiFiConnection() {
    if (WiFi.status() != WL_CONNECTED) {
        ...
        // Coba sambungkan beberapa kali
        while (attempts < 10 && WiFi.status() != WL_CONNECTED) {
            Serial.print('.');
            delay(1000);  // Delay untuk memberi waktu
            attempts++;
        }
...
void setup() {
    ...
    // Firebase Configuration (hanya jika WiFi terhubung)
    if (WiFi.status() == WL_CONNECTED) {
        ...
        Serial.println("Firebase initialization skipped due to no WiFi connection.");
    }
    Serial.println("------- CAN Read ----------");
    // Initialize mutex
    ...
    xTaskCreatePinnedToCore(readCANData, "TaskCANRead", 10000, NULL, 1, &TaskCANRead, 1); // Core 1
    xTaskCreatePinnedToCore(sendDataToFirebase, "TaskFirebase", 10000, NULL, 1, &TaskFirebase, 1); // Core 1
}

void loop() {
//    manageWiFiConnection();  // Pantau dan sambungkan WiFi
}
